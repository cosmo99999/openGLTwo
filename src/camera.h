...
enum Camera_Movement { FORWARD, BACKWARD, LEFT, RIGHT, NONE };
...
inline bool shouldClamp(float speed, float max) {
  if (speed > 0 && speed > max || speed < 0 && speed < max) {
    return true;
  }
  return false;
};
inline bool shouldClamp(float speed) {
  if (speed > 0 && speed - 5 < 0 || speed < 0 && speed + 5 > 0) {
    return true;
  }
  return false;
};
...
public:
  ...
  void UpdatePosition(std::vector<Camera_Movement> directions,
                      float deltaTime) {
  ...
  // systems)
  void ProcessKeyboard(std::vector<Camera_Movement> directions,
                       float deltaTime) {
